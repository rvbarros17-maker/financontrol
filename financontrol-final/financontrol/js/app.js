...
class FinanControl {
    ...
    setupEventListeners() {
        ...
        // Seletores de cor - Conta
        document.querySelectorAll('#accountColorPicker .color-option').forEach(btn => {
            btn.addEventListener('click', (e) => {
                e.preventDefault();
                const color = e.currentTarget.dataset.color;
                document.getElementById('accountColor').value = color;
                // Highlight selecionado
                document.querySelectorAll('#accountColorPicker .color-option').forEach(b => {
        ...
        // Seletores de cor - Cartão
        document.querySelectorAll('#cardColorPicker .color-option').forEach(btn => {
            btn.addEventListener('click', (e) => {
                e.preventDefault();
                const color = e.currentTarget.dataset.color;
                document.getElementById('cardColor').value = color;
                // Highlight selecionado
                document.querySelectorAll('#cardColorPicker .color-option').forEach(b => {
                    b.classList.remove('ring-4', 'ring-offset-2');
                    b.classList.add('border-2', 'border-gray-300');
                });
                e.currentTarget.classList.remove('border-2', 'border-gray-300');
    ...
    openAccountModal(account = null) {
        ...
        if (account) {
            ...
            // Highlight primeira cor por padrão
            document.querySelectorAll('#accountColorPicker .color-option').forEach((btn, index) => {
                btn.classList.remove('ring-4', 'ring-offset-2');
                btn.classList.add('border-2', 'border-gray-300');
                if (index === 0) {
                    btn.classList.remove('border-2', 'border-gray-300');
    ...
    openCardModal(card = null) {
        ...
        if (card) {
            ...
            // Highlight primeira cor por padrão
            document.querySelectorAll('#cardColorPicker .color-option').forEach((btn, index) => {
                btn.classList.remove('ring-4', 'ring-offset-2');
                btn.classList.add('border-2', 'border-gray-300');
                if (index === 0) {
                    btn.classList.remove('border-2', 'border-gray-300');
